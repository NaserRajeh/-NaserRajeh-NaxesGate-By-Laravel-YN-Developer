            ...
            <div class="col-12">
                ...
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">عرض الجامعات
                        </li>
                    </ol>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">قائمة بجميع الجامعات</h4>
                </div>
                <div class="card-content">
                    <div class="card-body card-dashboard">

                        @if($Unversity->count() == 0)
                                <div class="card-body">
                                    <div class="text-center">
                                        <img  width="200" src="{{URL::asset('app-assets/images/error-empty.png')}}" class="img-fluid">
                                        <p data-v-99f11192="">لا يوجد جامعات حتى الأن</p>
                                        <a href="{{route('Unversity.create') }}" class="btn btn-primary">إضافة جامعة </a>
                                    </div>
                                 </div>
                        @else
                                <div class="table-responsive">
                                    <table class="table table-striped table zero-configuration " id="ex_table" >
                                        ...
                                        <tbody>
                                            @foreach ($Unversity as $index => $Unversitys)
                                            <tr class="rowdate">
                                                <td>{{++$index}}</td>
                                                <td>{{$Unversitys->un_name}}</td>
                                                <td><a href="{{('https://www.'.$Unversitys->un_web.'.com') }}" target="/blank">{{ $Unversitys->un_web }}</a></td>
                                                <td>{{$Unversitys->created_at}}</td>
                                                <td>
                                                   <form method="POST" action="{{ route('Unversity.destroy',$Unversitys->un_id) }}">
                                                    {{ csrf_field() }}
                                                    {{ method_field('DELETE') }}
                                                    <a href="{{route('Unversity.edit',$Unversitys) }}"><i class="feather icon-edit"></i>{{ 'تعديل ' }}</a>
                                                    <a href="{{route('Unversity.show',$Unversitys) }}"><i class="feather icon-eye"></i>{{ ' عرض' }}</a>
                                                    <a href="{{ route('Unversity.destroy',$Unversitys->un_id) }}"><i class="feather icon-trash"></i>{{ ' حذف' }}</a>
                                                    </form> 
                                                </td>
                                            </tr>
                                            @endforeach
